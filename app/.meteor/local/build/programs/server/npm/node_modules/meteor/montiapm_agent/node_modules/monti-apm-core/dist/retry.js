...
function retry(promiser) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var attempt = function attempt(lastError) {
      if (++count > options.maxRetries) {
        var message = 'Reached maximum retry limit for ' + lastError.message;
        var err = new MaxRetryError(message);
        return reject(err);
      }

      // stop a few milliseconds between retries
      var millis = options.timeFunction(count);
      delay(millis).then(function () {
        return promiser();
